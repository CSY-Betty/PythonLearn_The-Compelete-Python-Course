...
def current_tab_unsaved():
    text_wiget = get_text_wiget()
    content = text_wiget.get("1.0", "end-1c")
    return hash(content) != text_contents[str(text_wiget)]
...
def open_file():
    ...
    creat_file(content, filename)


def show_about_info():
    messagebox.showinfo(
        title="About",
        message="The Text Editor ia s simple......"
    )


root = tk.Tk()
root.title('Text Editor Try')
...
root.bind("<KeyPress>", lambda event: check_for_change())
